# ART-0114: server-routers-terminal-py

## Artifact Identity
- ART ID: ART-0114
- Path: server/routers/terminal.py
- Category: BACKEND_SOURCE
- Contract Version: 1.0

## Purpose
Defines HTTP/WebSocket route behavior in `server/routers/terminal.py` and translates API surface contracts into service-layer execution. Primary symbols: TerminalCloseCode, validate_terminal_id, CreateTerminalRequest, RenameTerminalRequest, TerminalInfoResponse, list_project_terminals, create_project_terminal, rename_project_terminal.
...
## Acceptance Checks
- [ ] Artifact identity exactly matches `ART-0114` and `server/routers/terminal.py` in `.planning/specs/INVENTORY.md`.
