# ART-0106: server-routers-devserver-py

## Artifact Identity
- ART ID: ART-0106
- Path: server/routers/devserver.py
- Category: BACKEND_SOURCE
- Contract Version: 1.0

## Purpose
Defines HTTP/WebSocket route behavior in `server/routers/devserver.py` and translates API surface contracts into service-layer execution. Primary symbols: get_project_dir, validate_custom_command_strict, get_project_devserver_manager, validate_dev_command, get_devserver_status, start_devserver, stop_devserver, get_devserver_config.
...
## Acceptance Checks
- [ ] Artifact identity exactly matches `ART-0106` and `server/routers/devserver.py` in `.planning/specs/INVENTORY.md`.
