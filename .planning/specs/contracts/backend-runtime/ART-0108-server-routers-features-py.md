# ART-0108: server-routers-features-py

## Artifact Identity
- ART ID: ART-0108
- Path: server/routers/features.py
- Category: BACKEND_SOURCE
- Contract Version: 1.0

## Purpose
Defines HTTP/WebSocket route behavior in `server/routers/features.py` and translates API surface contracts into service-layer execution. Primary symbols: _get_db_classes, get_db_session, feature_to_response, list_features, create_feature, create_features_bulk, get_dependency_graph, get_feature.
...
## Acceptance Checks
- [ ] Artifact identity exactly matches `ART-0108` and `server/routers/features.py` in `.planning/specs/INVENTORY.md`.
