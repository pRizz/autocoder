# ART-0122: server-services-process-manager-py

## Artifact Identity
- ART ID: ART-0122
- Path: server/services/process_manager.py
- Category: BACKEND_SOURCE
- Contract Version: 1.0

## Purpose
Implements stateful backend service behavior in `server/services/process_manager.py` for process/session/control-plane lifecycle operations. Primary symbols: sanitize_output, AgentProcessManager, get_manager, cleanup_all_managers, cleanup_orphaned_locks.
...
## Acceptance Checks
- [ ] Artifact identity exactly matches `ART-0122` and `server/services/process_manager.py` in `.planning/specs/INVENTORY.md`.
