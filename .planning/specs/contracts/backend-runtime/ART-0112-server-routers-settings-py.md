# ART-0112: server-routers-settings-py

## Artifact Identity
- ART ID: ART-0112
- Path: server/routers/settings.py
- Category: BACKEND_SOURCE
- Contract Version: 1.0

## Purpose
Defines HTTP/WebSocket route behavior in `server/routers/settings.py` and translates API surface contracts into service-layer execution. Primary symbols: _parse_yolo_mode, get_available_providers, get_available_models, _parse_int, _parse_bool, get_settings, update_settings.
...
## Acceptance Checks
- [ ] Artifact identity exactly matches `ART-0112` and `server/routers/settings.py` in `.planning/specs/INVENTORY.md`.
