...
## Purpose
Implements backend data/domain contract logic in `api/database.py` that is consumed by server routes, services, and orchestration flows. Primary symbols: _utc_now, Base, Feature, Schedule, ScheduleOverride, get_database_path, get_database_url, _migrate_add_in_progress_column.
...
## Acceptance Checks
- [ ] Artifact identity exactly matches `ART-0075` and `api/database.py` in `.planning/specs/INVENTORY.md`.
