# ART-0104: server-routers-agent-py

## Artifact Identity
- ART ID: ART-0104
- Path: server/routers/agent.py
- Category: BACKEND_SOURCE
- Contract Version: 1.0

## Purpose
Defines HTTP/WebSocket route behavior in `server/routers/agent.py` and translates API surface contracts into service-layer execution. Primary symbols: _get_settings_defaults, get_project_manager, get_agent_status, start_agent, stop_agent, pause_agent, resume_agent, graceful_pause_agent.
...
## Acceptance Checks
- [ ] Artifact identity exactly matches `ART-0104` and `server/routers/agent.py` in `.planning/specs/INVENTORY.md`.
