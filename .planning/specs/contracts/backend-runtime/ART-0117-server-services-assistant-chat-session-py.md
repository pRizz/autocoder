# ART-0117: server-services-assistant-chat-session-py

## Artifact Identity
- ART ID: ART-0117
- Path: server/services/assistant_chat_session.py
- Category: BACKEND_SOURCE
- Contract Version: 1.0

## Purpose
Implements stateful backend service behavior in `server/services/assistant_chat_session.py` for process/session/control-plane lifecycle operations. Primary symbols: get_system_prompt, AssistantChatSession, get_session, create_session, remove_session, list_sessions, cleanup_all_sessions.
...
## Acceptance Checks
- [ ] Artifact identity exactly matches `ART-0117` and `server/services/assistant_chat_session.py` in `.planning/specs/INVENTORY.md`.
