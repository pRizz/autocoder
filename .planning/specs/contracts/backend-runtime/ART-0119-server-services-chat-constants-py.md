# ART-0119: server-services-chat-constants-py

## Artifact Identity
- ART ID: ART-0119
- Path: server/services/chat_constants.py
- Category: BACKEND_SOURCE
- Contract Version: 1.0

## Purpose
Implements stateful backend service behavior in `server/services/chat_constants.py` for process/session/control-plane lifecycle operations. Primary symbols: check_rate_limit_error, safe_receive_response, make_multimodal_message.
...
## Acceptance Checks
- [ ] Artifact identity exactly matches `ART-0119` and `server/services/chat_constants.py` in `.planning/specs/INVENTORY.md`.
