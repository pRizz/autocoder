...
## Purpose
Implements backend runtime behavior in `client.py` as part of the AutoForge control-plane execution stack. Primary symbols: convert_model_for_vertex, get_extra_read_paths, create_client.
...
## Direct Dependencies
- ART-0100 `security.py`: Direct runtime collaboration.
...
## Acceptance Checks
- [ ] Artifact identity exactly matches `ART-0082` and `client.py` in `.planning/specs/INVENTORY.md`.
