# ART-0109: server-routers-filesystem-py

## Artifact Identity
- ART ID: ART-0109
- Path: server/routers/filesystem.py
- Category: BACKEND_SOURCE
- Contract Version: 1.0

## Purpose
Defines HTTP/WebSocket route behavior in `server/routers/filesystem.py` and translates API surface contracts into service-layer execution. Primary symbols: get_blocked_paths, is_path_blocked, is_hidden_file, matches_blocked_pattern, is_unc_path, list_directory, list_drives, get_windows_drives.
...
## Acceptance Checks
- [ ] Artifact identity exactly matches `ART-0109` and `server/routers/filesystem.py` in `.planning/specs/INVENTORY.md`.
