# ART-0123: server-services-project-config-py

## Artifact Identity
- ART ID: ART-0123
- Path: server/services/project_config.py
- Category: BACKEND_SOURCE
- Contract Version: 1.0

## Purpose
Implements stateful backend service behavior in `server/services/project_config.py` for process/session/control-plane lifecycle operations. Primary symbols: _validate_project_dir, ProjectConfig, _get_config_path, _load_config, _save_config, _parse_package_json, _is_poetry_project, detect_project_type.
...
## Acceptance Checks
- [ ] Artifact identity exactly matches `ART-0123` and `server/services/project_config.py` in `.planning/specs/INVENTORY.md`.
