# ART-0128: server-utils-process-utils-py

## Artifact Identity
- ART ID: ART-0128
- Path: server/utils/process_utils.py
- Category: BACKEND_SOURCE
- Contract Version: 1.0

## Purpose
Provides reusable backend utility logic in `server/utils/process_utils.py` used by routers/services for validation and process safety. Primary symbols: KillResult, kill_process_tree.
...
## Acceptance Checks
- [ ] Artifact identity exactly matches `ART-0128` and `server/utils/process_utils.py` in `.planning/specs/INVENTORY.md`.
