# ART-0130: server-utils-validation-py

## Artifact Identity
- ART ID: ART-0130
- Path: server/utils/validation.py
- Category: BACKEND_SOURCE
- Contract Version: 1.0

## Purpose
Provides reusable backend utility logic in `server/utils/validation.py` used by routers/services for validation and process safety. Primary symbols: is_valid_project_name, validate_project_name.
...
## Acceptance Checks
- [ ] Artifact identity exactly matches `ART-0130` and `server/utils/validation.py` in `.planning/specs/INVENTORY.md`.
