# ART-0111: server-routers-schedules-py

## Artifact Identity
- ART ID: ART-0111
- Path: server/routers/schedules.py
- Category: BACKEND_SOURCE
- Contract Version: 1.0

## Purpose
Defines HTTP/WebSocket route behavior in `server/routers/schedules.py` and translates API surface contracts into service-layer execution. Primary symbols: _schedule_to_response, _get_db_session, list_schedules, create_schedule, get_next_scheduled_run, get_schedule, update_schedule, delete_schedule.
...
## Acceptance Checks
- [ ] Artifact identity exactly matches `ART-0111` and `server/routers/schedules.py` in `.planning/specs/INVENTORY.md`.
