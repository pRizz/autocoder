...
## Purpose
Implements backend runtime behavior in `server/schemas.py` as part of the AutoForge control-plane execution stack. Primary symbols: ProjectCreate, ProjectStats, ProjectSummary, ProjectDetail, ProjectPrompts, ProjectPromptsUpdate, ProjectSettingsUpdate, FeatureBase.
...
## Acceptance Checks
- [ ] Artifact identity exactly matches `ART-0115` and `server/schemas.py` in `.planning/specs/INVENTORY.md`.
