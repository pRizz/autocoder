...
## Purpose
Defines backend regression and security assertions for `test_security.py` to protect behavior contracts during regeneration and migration. Primary symbols: temporary_home, check_hook, test_extract_commands, test_validate_chmod, test_validate_init_script, test_pattern_matching, test_yaml_loading, test_command_validation.
...
- ART-0109 `server/routers/filesystem.py`: Primary module under test.
...
## Acceptance Checks
- [ ] Artifact identity exactly matches `ART-0143` and `test_security.py` in `.planning/specs/INVENTORY.md`.
