...
## Purpose
Exposes MCP tool-server behavior in `mcp_server/__init__.py` so agent processes can query and mutate project feature state.
...
## Acceptance Checks
- [ ] Artifact identity exactly matches `ART-0089` and `mcp_server/__init__.py` in `.planning/specs/INVENTORY.md`.
