# ART-0126: server-services-terminal-manager-py

## Artifact Identity
- ART ID: ART-0126
- Path: server/services/terminal_manager.py
- Category: BACKEND_SOURCE
- Contract Version: 1.0

## Purpose
Implements stateful backend service behavior in `server/services/terminal_manager.py` for process/session/control-plane lifecycle operations. Primary symbols: TerminalInfo, _get_shell, TerminalSession, create_terminal, list_terminals, rename_terminal, delete_terminal, get_terminal_session.
...
## Acceptance Checks
- [ ] Artifact identity exactly matches `ART-0126` and `server/services/terminal_manager.py` in `.planning/specs/INVENTORY.md`.
