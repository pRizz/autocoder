# ART-0124: server-services-scheduler-service-py

## Artifact Identity
- ART ID: ART-0124
- Path: server/services/scheduler_service.py
- Category: BACKEND_SOURCE
- Contract Version: 1.0

## Purpose
Implements stateful backend service behavior in `server/services/scheduler_service.py` for process/session/control-plane lifecycle operations. Primary symbols: SchedulerService, get_scheduler, cleanup_scheduler.
...
## Acceptance Checks
- [ ] Artifact identity exactly matches `ART-0124` and `server/services/scheduler_service.py` in `.planning/specs/INVENTORY.md`.
