...
## Purpose
Implements backend runtime behavior in `auth.py` as part of the AutoForge control-plane execution stack. Primary symbols: is_auth_error, print_auth_error_help.
...
## Acceptance Checks
- [ ] Artifact identity exactly matches `ART-0078` and `auth.py` in `.planning/specs/INVENTORY.md`.
