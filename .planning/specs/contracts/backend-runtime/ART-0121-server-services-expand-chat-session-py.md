# ART-0121: server-services-expand-chat-session-py

## Artifact Identity
- ART ID: ART-0121
- Path: server/services/expand_chat_session.py
- Category: BACKEND_SOURCE
- Contract Version: 1.0

## Purpose
Implements stateful backend service behavior in `server/services/expand_chat_session.py` for process/session/control-plane lifecycle operations. Primary symbols: ExpandChatSession, get_expand_session, create_expand_session, remove_expand_session, list_expand_sessions, cleanup_all_expand_sessions.
...
## Acceptance Checks
- [ ] Artifact identity exactly matches `ART-0121` and `server/services/expand_chat_session.py` in `.planning/specs/INVENTORY.md`.
