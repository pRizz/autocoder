# ART-0004: claude-commands-check-code-md

## Artifact Identity
- ART ID: ART-0004
- Path: .claude/commands/check-code.md
- Category: OPS_AUTOMATION
- Contract Version: 1.0

## Purpose
Defines agent command/skill/prompt automation behavior in `.claude/commands/check-code.md` to standardize repository workflows. Key sections: Python linting, Security tests, ESLint (will fail until we add the config), TypeScript check + build.
...
## Acceptance Checks
- [ ] Artifact identity exactly matches `ART-0004` and `.claude/commands/check-code.md` in `.planning/specs/INVENTORY.md`.
