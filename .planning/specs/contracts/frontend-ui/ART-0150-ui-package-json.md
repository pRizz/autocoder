...
## Purpose
Defines frontend build/lint/test/runtime configuration behavior in `ui/package.json` for deterministic local and CI execution.
...
## Direct Dependencies
- ART-0148 `ui/eslint.config.js`: Lint command and static-analysis behavior.
- ART-0151 `ui/playwright.config.ts`: E2E test command and runtime dependencies.
...
## Acceptance Checks
- [ ] Artifact identity exactly matches `ART-0150` and `ui/package.json` in `.planning/specs/INVENTORY.md`.
