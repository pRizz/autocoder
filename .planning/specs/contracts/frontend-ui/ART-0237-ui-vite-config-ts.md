...
## Purpose
Defines frontend build/lint/test/runtime configuration behavior in `ui/vite.config.ts` for deterministic local and CI execution.
...
## Acceptance Checks
- [ ] Artifact identity exactly matches `ART-0237` and `ui/vite.config.ts` in `.planning/specs/INVENTORY.md`.
