# ART-0151: ui-playwright-config-ts
...
## Purpose
Defines frontend build/lint/test/runtime configuration behavior in `ui/playwright.config.ts` for deterministic local and CI execution.
...
## Acceptance Checks
- [ ] Artifact identity exactly matches `ART-0151` and `ui/playwright.config.ts` in `.planning/specs/INVENTORY.md`.
