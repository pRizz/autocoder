# ART-0236: ui-tsconfig-node-json
...
## Purpose
Defines frontend build/lint/test/runtime configuration behavior in `ui/tsconfig.node.json` for deterministic local and CI execution.
...
## Acceptance Checks
- [ ] Artifact identity exactly matches `ART-0236` and `ui/tsconfig.node.json` in `.planning/specs/INVENTORY.md`.
