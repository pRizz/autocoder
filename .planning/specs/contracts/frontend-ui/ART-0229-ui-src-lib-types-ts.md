...
## Purpose
Provides shared frontend utility/API/type behavior in `ui/src/lib/types.ts` for component and hook contracts. Primary symbols: AGENT_MASCOTS, FeatureStatus, AgentStatus, DevServerStatus, AgentMascot, AgentState, AgentType, OrchestratorState.
...
## Acceptance Checks
- [ ] Artifact identity exactly matches `ART-0229` and `ui/src/lib/types.ts` in `.planning/specs/INVENTORY.md`.
